...
<script setup>
  ...
  const store = () => {
    form.post(route('companies.subscriptions.resume.store', props.model.company), {
      onSuccess: () => {
        //
      },
    });
...
<template>
  <Head :title="`${__('app.labels.resume_subscription')} | ${model.company.name}`" />
    ...
    <div class="w-full flex flex-col md:flex-row md:items-center gap-4 mb-3">
      ...
      <div class="flex flex-col gap-3 mr-auto">
        <h1 class="text-slate-800 text-2xl font-hairline leading-tight">
          <span class="font-semibold">{{ __('app.labels.resume_subscription') }}</span>
        </h1>
      </div>
    </div>
