<script setup>
  ...
  const {
    ...
    hasFilter,
    hasSelectedFilters,
  } = useFilters(model.value.filters, 'admin.permissions.index');
  ...
  const confirmDelete = (data) => {
    $eventBus.emit('show-confirmation-dialog-modal', {
      ...
      message: `Are you sure you want to delete '${data.name}' from permissions?`,
      callback: () => {
        destroyActionForm.delete(route('admin.permissions.destroy', data), {
          onSuccess: () => {
...
<template>
  ...
  >
    <vue-good-table
      :columns="columns"
      :rows="model.permissions.data"
      :total-rows="model.permissions.meta.total"
      :pagination-options="{
        enabled: true,
        perPage: model.permissions.meta.per_page,
        total: model.permissions.meta.total,
        setCurrentPage: model.permissions.meta.current_page,
      ...
      <template #table-row="props">
        ...
        >
          ...
          >
            ...
            <AppButton
              ...
              :href="route('admin.permissions.edit', props.row)"
              plain
            >
        ...
        >
          <a
            ...
            href="#"
            @click.prevent="$eventBus.emit('show-permission', props.row)"
          >
