<script setup>
  ...
  const confirmDelete = (data) => {
    $eventBus.emit('show-confirmation-dialog-modal', {
      ...
      message: `Are you sure you want to delete '${data.name}' from features?`,
      callback: () => {
        destroyActionForm.delete(route('admin.features.destroy', data), {
          onSuccess: () => {
...
<template>
  ...
  >
    <vue-good-table
      :columns="columns"
      :rows="model.features.data"
      :total-rows="model.features.meta.total"
      :pagination-options="{
        enabled: true,
        perPage: model.features.meta.per_page,
        total: model.features.meta.total,
        setCurrentPage: model.features.meta.current_page,
    ...
    >
      <!-- Table Actions -->
      <template v-if="model.filters" #table-actions>
        ...
        >
          ...
          >
            ...
            <AppButton
              ...
              :href="route('admin.features.edit', props.row)"
              plain
            >
