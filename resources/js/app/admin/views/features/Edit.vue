...
<script setup>
  ...
  const form = useForm({
    name: props.model.feature.name,
    key: props.model.feature.key,
    description: props.model.feature.description,
    trial_limit: props.model.feature.trial_limit,
    is_unlimited: props.model.feature.is_unlimited,
    usable: props.model.feature.usable,
  });
  ...
  const store = () => {
    ...
    form
      ...
      .patch(route("admin.features.update", props.model.feature), {
        //
      });
  };
</script>
...
<template>
  ...
  <div class="flex flex-col gap-10 relative">
    <!-- Heading Area -->
    <div class="w-full flex md:items-start justify-between px-6 mb-3">
      <!-- Heading -->
      <div class="mr-auto space-y-2">
        <h1 class="text-slate-800 text-2xl font-medium leading-tight">{{ __('app.labels.edit_feature') }}</h1>

        <p class="text-sm font-medium">{{ __('app.labels.update_feature_prompt') }}</p>
      </div>
      ...
      <form action="#" @submit.prevent="store">
        ...
        <div class="mt-4">
          <AppInputLabel class="inline-flex items-center gap-2">
            <AppCheckbox v-model="form.usable" />{{ __('app.labels.active') }}
          </AppInputLabel>
        </div>
        <!-- END Usable -->
        ...
        <div class="mt-4">
          <AppInputLabel class="inline-flex items-center gap-2">
            <AppCheckbox v-model="form.is_unlimited" />{{ __('app.labels.is_unlimited') }}
          </AppInputLabel>
        </div>
        <!-- END Is Unlimited -->
        ...
        <div class="mt-4">
          ...
          <AppInput id="key" v-model="form.key" class="w-full mt-2" name="key" disabled />
