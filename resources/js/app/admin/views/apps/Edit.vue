...
<script setup>
  ...
  const addFeature = (feature) => {
    ...
    form.features = state.features.filter((feature) => planIsEditable(feature));
  };
  ...
  const addPlan = (plan) => {
    ...
    form.plans = state.plans.filter((plan) => planIsEditable(plan));
  };
  ...
  const planIsEditable = (plan) => {
    return _.startsWith(plan.id, '_');
  };

  const store = () => {
    form.clearErrors();
    scrollTo('body', 500, { container: 'main', offset: -200 });
    ...
    form
      ...
      .post(route('admin.apps.update', props.app), {
        onSuccess: () => {
...
<template>
  <Head title="Edit App - Apps | Admin" />

  <div class="flex flex-col gap-10 relative">
    <!-- heading area -->
    <div class="w-full flex items-center flex-wrap gap-y-6 mb-3">
      <!-- Heading -->
      <h1 class="text-slate-800 text-2xl font-hairline leading-tight mr-auto">
        <span class="font-semibold">Editing App: {{ app.name }}</span>
      </h1>
      ...
      <div class="flex items-center gap-3">
        ...
        <AppButton @click.prevent="$eventBus.emit('show-modal', 'planCreateModal')"> Add Billing Plan </AppButton>
      </div>
    </div>

    <!-- Content Area -->
    <div class="w-full min-w-full px-6 md:px-0 py-6">
      <!-- Form -->
      <div>
        <form
          action="#"
        ...
        >
          ...
          <!-- Logo -->
          <div class="mt-4">
            <AppInputLabel for="logo">
              Logo
              <a
                v-if="form.logo != null"
                class="ml-2 text-blue-500 hover:underline focus:ring"
                href="#"
                @click.prevent="form.logo = null"
              >
                Remove logo
              </a>
            </AppInputLabel>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
            ...
            <template v-if="features_count > 0">
              <div
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
            <AppPlanCard
              v-for="(plan, index) in state.plans"
              :key="plan.id"
              :plan="plan"
            >
              <template
                v-if="planIsEditable(plan)"
                #footer
              >
                <AppButton
                  type="button"
