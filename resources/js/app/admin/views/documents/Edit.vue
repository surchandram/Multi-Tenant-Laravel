...
<script setup>
  ...
  const form = useForm({
    title: props.model.document.title,
    type_id: props.model.document.type?.id,
    body: props.model.document.body,
    usable: props.model.document.usable,
  });
  ...
  const store = () => {
    ...
    form
      ...
      .patch(route("admin.documents.update", props.model.document), {
        //
      });
  };
</script>
...
<template>
  <Head :title="`${__('admin.labels.edit_document')} | ${__('admin.labels.dashboard_title')}`" />
