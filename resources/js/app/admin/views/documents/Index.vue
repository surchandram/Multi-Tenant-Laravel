...
<script setup>
  ...
  const {
    ...
    hasSelectedFilters,
    selectedLabels,
  } = useFilters(props.model.filters, 'admin.documents.index');
  ...
  onMounted(() => {
    ...
    watch(showFilters, (val) => {
      if (val == true) {
        setTimeout(() => {
          $scrollTo('#filtersWrapper', { container: 'body' });
        }, 500);
      }
...
<template>
  <Head :title="`${__('admin.labels.documents')} | ${__('admin.labels.dashboard_title')}`" />
  ...
  <div class="flex flex-col gap-10 relative">
    ...
    <div class="w-full flex items-center mb-3">
      ...
      <div class="flex items-center gap-3">
        <AppSuccessButton :href="route('admin.documents.create')">
          <i class="bi bi-file-plus"></i> {{ __('admin.labels.add_document') }}
        </AppSuccessButton>
      ...
      >
        ...
        <div v-if="showFilters" class="p-4 bg-white">
          ...
          <div class="grid md:grid-cols-3 gap-4">
            <!-- Filter Map -->
            <div :key="`filterMap${key}`" v-for="(filterMap, key) in filters" class=" col-span full md:col-span-1">
              <!-- Filter Map Heading -->
