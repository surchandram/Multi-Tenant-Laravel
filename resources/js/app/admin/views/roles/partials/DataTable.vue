<script setup>
  ...
  const {
    ...
    hasFilter,
    hasSelectedFilters,
  } = useFilters(model.value.filters, 'admin.roles.index');
  ...
  const confirmDelete = (data) => {
    $eventBus.emit('show-confirmation-dialog-modal', {
      ...
      message: `Are you sure you want to delete '${data.name}' from roles?`,
      callback: () => {
        destroyActionForm.delete(route('admin.roles.destroy', data), {
          onSuccess: () => {
...
<template>
  ...
  >
    <vue-good-table
      :columns="columns"
      :rows="model.roles"
      :total-rows="model.roles.length"
      :pagination-options="{
        enabled: true,
        perPage: model.meta.per_page,
        total: model.roles.length,
    ...
    >
      ...
      <template #table-row="props">
        ...
        >
          ...
          >
            ...
            <AppButton
              ...
              :href="route('admin.roles.edit', props.row)"
              plain
            >
        ...
        <!-- Permissions count Field -->
        <span v-else-if="props.column.field == 'permissions_count' && props.row.children_count > 1">
          &nbsp;
        </span>

        <!-- Status Field -->
        <span v-else-if="props.column.field == 'usable' && props.row.children_count > 1">
          &nbsp;
        </span>

        <!-- Status Field with parent -->
        <div v-else-if="props.column.field == 'usable' && props.row.children_count < 1">
          <span
