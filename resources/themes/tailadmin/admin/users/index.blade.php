...
@section('content')
    <div class="flex items-center justify-between">
        ...
        <div class="btn-group" role="group">
            <x-link-button themed size="sm" light rounded="md" href="{{ route('admin.users.create') }}">
                {{ __('Add a user') }}
            </x-link-button>
        </div>
    </div>
                    ...
                    <x-table-row striped>
                        ...
                        <x-table-col>
                            <div class="flex items-center gap-2" role="group">
                                <x-link-button themed size="sm" light rounded="md" href="{{ route('admin.users.edit', $user) }}"
                                    class="">
                                    {{ __('Edit') }}
                                </x-link-button>
                                ...
                                <form id="del-user-{{ $user->id }}-form"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <x-link-button themed size="sm" light rounded="md" href="{{ route('admin.users.destroy', $user) }}"
                                        bg="bg-danger"
