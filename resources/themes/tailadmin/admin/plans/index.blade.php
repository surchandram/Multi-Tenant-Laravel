...
@section('content')
    <div class="flex items-center justify-between">
        ...
        <aside class="btn-group">
            <x-link-button themed size="sm" light rounded="md" ring href="{{ route('admin.plans.create') }}" class="btnprimary">
                {{ __('Add a plan') }}
            </x-link-button>
    ...
    @if($plans->count())
        <x-card clear-width no-pad no-wrap-pad class="mb-5">
            <x-table-responsive responsive class="-py-2">
                <x-slot name="header">
                    ...
                    <x-table-row hoverable striped>
                        ...
                        <x-table-col>
                            <div class="flex items-center gap-2" role="group">
                                <x-link-button themed size="sm" light rounded="md" ring href="{{ route('admin.plans.edit', $plan) }}"
                                    variant="blue">
                                    {{ __('Edit') }}
                                </x-link-button>
                                <x-link-button themed size="sm" light rounded="md" ring href="{{ route('admin.plans.features.index', $plan) }}"
                                    bg="teal">
                                    {{ __('Features') }}
                                </x-link-button>
                                <x-link-button themed size="sm" light rounded="md" ring href="{{ route('admin.plans.destroy', $plan) }}"
                                    bg="bg-danger"
