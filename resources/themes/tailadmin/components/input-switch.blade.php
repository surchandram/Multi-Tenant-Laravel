...
<div
    x-data="{
        id: @js($id),
        switcherToggle: @js($value),
        init() {
            this.$watch('switcherToggle', (val) => {
        ...
        changed($event) {
            ...
            this.switcherToggle = !this.switcherToggle
            setTimeout(() => {
                $dispatch('switcher-toggled', {
                    id: this.id,
                    value: this.switcherToggle
                })
                $refs[@js($id)]?._x_model?.set(this.switcherToggle)
            }, 500)
        }
    }"
    {{ $attributes }}
    x-on:switcher-revert.window="revert"
>
    <label for="{{ $id }}" class="flex cursor-pointer select-none items-center">
        <div class="relative">
            <input type="checkbox" id="{{ $id }}" value="{{ $value }}" class="sr-only" @change.once.debounce="changed" />
